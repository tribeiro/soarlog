...
from dbComF import soarDB
from soarlogF_DataQuality import DataQuality
from soarlogF_DataTransfer import DataTransfer
...
#
            ...
            if os.path.isfile(frame):
                ...
                try:
                    if query.INSTRUME == 'SOI':
                        ...
                        query2 = session_CID.query(self.Obj_INSTRUMENTS['Spartan IR Camera']).filter(
                            self.Obj_INSTRUMENTS['Spartan IR Camera'].FILENAME.like(frame))[0]
                        zoom = d.get('zoom')
                        pan = d.get('pan')
    ...
    def initGUI(self):
        ...
        tm = SOLogTableModel(self.getTableData(), databaseF.frame_infos.tvDB.keys(), self,
                             commitDB=self.emmitTableDataChanged)
        ...
        self.connect(self.ui.tableDB, QtCore.SIGNAL("selectionChanged(const QItemSelection&, const QItemSelection&)"),
        ...
        try:
            SavedInfoOrder = np.loadtxt(os.path.join(self._CFGFilePath_, self._CFGFiles_['OrderInfo']), dtype='int',
                                        unpack=True)
...
################################################################################################
    ...
    def OpenPreferences(self):
        ...
        tab1 = pref_ui.tabWidget.widget(
            0)  # .gridLayout.tableView.setModel(tableModel) #.tab.gridLayout.tableView.setModel(tableModel)
    ...
    def emmitTableDataChanged(self, index):
        ...
        try:
            ...
            self.emit(QtCore.SIGNAL("TableDataChanged(QModelIndex,QString)"), new_index,
                      self.ui.tableDB.model().data(new_index).toString())
...
''' % (len(calib))
    ...
    def getProjects(self):
        ...
        session_CID = self.Session()
        query = session_CID.query(self.Obj_CID).filter(self.Obj_CID.FILENAME.like('%SO%')).filter(
            self.Obj_CID.IMAGETYP.like('OBJECT'))[:]
...
################################################################################################
            ...
            for itr in range(len(query)):
                ...
                if frame.INSTRUME == 'Spartan IR Camera':
                    try:
                        frame2 = session_CID.query(self.Obj_INSTRUMENTS['Spartan IR Camera']).filter(
                            self.Obj_INSTRUMENTS['Spartan IR Camera'].FILENAME.like(
                                os.path.join(frame.PATH, frame.FILENAME)))[0]
                        if frame2.DETSERNO != '66':
                            writeFlag = False
                    except:
                        logging.debug('Error %s on frame %s...' % (sys.exc_info()[1], frame.FILENAME))
                ...
                if frame.INSTRUME == 'NOTE':
                    log = logNOTE
                if writeFlag:
                    outlog += log % {'time': time, 'FILENAME': os.path.basename(frame.FILENAME), 'OBJECT': frame.OBJECT,
                                     'OBSNOTES': frame.OBSNOTES, \
                                     'AIRMASS': frame.AIRMASS, 'EXPTIME': frame.EXPTIME, 'SEEING': frame.SEEING}
                if frame.INSTRUME == 'Goodman Spectrograph':
                    # frame2 = session_CID.query(self.Obj_CID).filter(self.Obj_CID.FILENAME.like(frame.FILENAME))[0]
                    logGS = '\tCONF: %s SLIT: %s OBSTYPE: %s\n' % (frame.SP_CONF, frame.SLIT, frame.IMAGETYP)
    ...
    def calcTime(self, id):
        ...
        session_CID = self.Session()
        query = session_CID.query(self.Obj_CID.id, self.Obj_CID.FILENAME, self.Obj_CID.DATEOBS, self.Obj_CID.TIMEOBS,
                                  self.Obj_CID.OBJECT, self.Obj_CID.EXPTIME, self.Obj_CID.IMAGETYP).filter(
            self.Obj_CID.FILENAME.like('%SO%')).filter(self.Obj_CID.OBJECT != "NOTE").filter(
            self.Obj_CID.IMAGETYP != "FAILED")[:]
        time_end = []
...
################################################################################################
            ...
            # print id, [ time[i] for i in time_start ], [time[i] for i in time_end], '%02.0f:%02.0f' %( np.floor(calcT), (calcT-np.floor(calcT))*60)
            logging.debug(self.semester_ID % (id) + ' %(hora)02.0f:%(min)02.0f (with %(nsub)i subblocks).' % {
                'nsub': len(time_start), 'hora': np.floor(calcT), 'min': (calcT - np.floor(calcT)) * 60})
        except:
            logging.debug('Failed to obtain program time')
        ...
        if len(indexes) > 0:
            ...
            for i in range(len(indexes)):
                if indexes[i].row() not in workRow:
                    ...
                    workIndexes.append(
                        self.ui.tableDB.model().sourceModel().createIndex(indexes[i].row(), self.CommentColumn))
            ...
            try:
                if query.INSTRUME == 'SOI':
                    ...
                    if self.ui.actionSpartan_showall.isChecked():
                        ...
                        frame2 = frame[:detIndex - 1] + '%.fits'
                        query2 = session_CID.query(self.Obj_INSTRUMENTS['Spartan IR Camera']).filter(
                            self.Obj_INSTRUMENTS['Spartan IR Camera'].FILENAME.like(frame2))[::]
    ...
    def filterTable(self):
        ...
        if self.searchInitialized:
            ...
            self.ui.tableDB.model().setFilterRegExp(
                QtCore.QRegExp(str(self.ui.lineFrameComment.text()), QtCore.Qt.CaseInsensitive,
        ...
        for i in range(record.count()):
            if not record.field(i).isNull():
                if record.field(i).name() == 'FILENAME':
                    if os.path.join(str(rr.PATH), str(rr.FILENAME)) != os.path.join(str(rr.PATH), str(
                            record.field(i).value().toString())):
                        shutil.copy(os.path.join(str(rr.PATH), str(rr.FILENAME)),
                                    os.path.join(str(rr.PATH), str(record.field(i).value().toString())))
...
################################################################################################
    ...
    def readCalibrations(self):
        ...
        qq = sMaster.query(self.Obj_CDQ).filter(
            self.Obj_CDQ.PID == str(self.calibhelp_ui.Instrument.currentText())).filter(
            self.Obj_CDQ.DATASET == str(self.calibhelp_ui.date.currentText()))[:]
